...
+ (DBWorker*)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        _singleton = [[DBWorker alloc] init];
    });
    return _singleton;
}
...
{
    return [DBWorker sharedInstance].bgProcessBDWorkingQueue;
}
...
- (void)initWithCompletionBlock:(void(^)(BOOL success))block
{
    ...
    dispatch_async( self.bgProcessBDWorkingQueue, ^{
        ...
        NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"MailDB" withExtension:@"momd"];
        ...
        do {
            ...
            storeDirURL = [storeDirURL URLByAppendingPathComponent:@"storeMailV6" isDirectory:YES];
            [[NSFileManager defaultManager] createDirectoryAtURL:storeDirURL withIntermediateDirectories:NO attributes:nil error:nil];
            [DBWorker addSkipBackupAttributeToItemAtURL:storeDirURL];
            
            NSURL *storeURL = [storeDirURL URLByAppendingPathComponent:@"MailDB.sqlite"];
#ifdef DEBUG
            NSLog(@"SqlLiteMail: %@", storeURL.path);
#endif
...
{
    return [DBWorker sharedInstance]._defaultManagedObjectContext;
}

+ (NSManagedObjectContext*)mocPerThread
{
    DBWorker *_self = [DBWorker sharedInstance];
...
{
    NSManagedObjectContext *context = [DBWorker mocPerThread];
